...
def create_sphere(x0, y0, z0, radius, step=1.0):
    print("Creating spheres around reference points...")
    mini_spheres = []
...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        print("Usage: python predict_and_cluster.py <background_pdb_file_path> <reference_pdb_file_path>")
        sys.exit(1)

    background_pdb_file_path = sys.argv[1]
    reference_pdb_file_path = sys.argv[2]
    model_path = os.path.join('trained_modules', 'intrusion_model.keras')
    ...
    background_coords, atoms = parse_pdb_file(background_pdb_file_path)
    reference_coords, _ = parse_pdb_file(reference_pdb_file_path)
    print(f"Parsed {len(background_coords)} background coordinates from PDB file.")
    print(f"Parsed {len(reference_coords)} reference coordinates from PDB file.")
    ...
    print("Generating spheres and making predictions for reference points...")
    num_ref_points = len(reference_coords)
    for i, ref_point in enumerate(reference_coords):
        print(f"Processing reference point {i+1}/{num_ref_points}...")
        x0, y0, z0 = ref_point
